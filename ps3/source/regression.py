...
class PolynomialRegression() :
    ...
    def fit(self, X, y) :
        ...
        ### ========== TODO : START ========== ###
        # part e: update theta (self.coef_) using closed-form solution
...
def main() :
    ...
    # test part b -- soln: [[1 1]
    #                       [1 2]
    #                       [1 3]]
    print(f'features:\n{model.generate_polynomial_features(X)}')
    
    # test part c -- soln: [4 7 10]
    model.coef_ = coef
    print(f'predictions: {model.predict(X)}')
    
    # test part d, bullet 1 -- soln: 10
    ...
    # test part e -- soln: theta = [ 2.44640709 -2.81635359]
    start = time.process_time()
    ...
    # test part h -- soln: 31.144823004794873
    model.coef_ = coefm
    print(f'RMSE: {model.rms_error(X, y)}')
    
    # part i -- RMSE vs model complexity (polynomial degree)
    m_vec = range(11)
    RMSE_train = []
    RMSE_test = []
